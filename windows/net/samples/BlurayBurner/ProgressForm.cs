...
namespace BluRayBurner.NET
{
	...
	public class ProgressForm : System.Windows.Forms.Form
	{
		...
		private void SetActualWriteSpeed(int speed)
		{
			...
			double dvdSpeed = (double)speed / Speed1xKB.DVD;
			double bdSpeed = (double)speed / Speed1xKB.BD;

			labelSpeed.Text = string.Format("{0} KB/s (CD: {1:#0.00}x DVD: {2:#0.00}x BD: {3:#0.00}x)", speed, cdSpeed, dvdSpeed, bdSpeed);
		}
