...
void CBurnerDialog::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_MODE, m_comboWriteMethod);
	...
	DDX_Control(pDX, IDC_CHECK_DVD_VIDEO, m_chkVideoDVD);
}
...
BEGIN_MESSAGE_MAP(CBurnerDialog, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_CREATE, OnButtonBurnClicked)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_CHECK_DVD_VIDEO, OnCheckDvdVideoClicked)
END_MESSAGE_MAP()
...
BOOL CBurnerDialog::OnInitDialog()
{
	...
	try
	{
		...
		// Write Method
		m_comboWriteMethod.InsertString(0, _TEXT("Disk-At-Once"));	m_comboWriteMethod.SetItemData(0, WriteMethod::DVDDao);
		m_comboWriteMethod.InsertString(1, _TEXT("Incremental"));	m_comboWriteMethod.SetItemData(1, WriteMethod::DVDIncremental);
		m_comboWriteMethod.SetCurSel(1);

		// Image Types
		m_comboImageType.InsertString(0, _TEXT("ISO9660"));			m_comboImageType.SetItemData(0, ImageTypeFlags::Iso9660);
		m_comboImageType.InsertString(1, _TEXT("Joliet"));			m_comboImageType.SetItemData(1, ImageTypeFlags::Joliet);
		m_comboImageType.InsertString(2, _TEXT("UDF"));				m_comboImageType.SetItemData(2, ImageTypeFlags::Udf);
		m_comboImageType.InsertString(3, _TEXT("UDF & ISO9660"));	m_comboImageType.SetItemData(3, ImageTypeFlags::UdfIso);
		m_comboImageType.InsertString(4, _TEXT("UDF & Joliet"));	m_comboImageType.SetItemData(4, ImageTypeFlags::UdfJoliet);
		m_comboImageType.SetCurSel(0);

		// Write parameters
		m_chkTest.SetCheck(0);
		m_chkEject.SetCheck(0);
		m_chkCloseDisc.SetCheck(0);

		// Erasing/ Formatting
		m_chkQuickErase.SetCheck(1);
		m_chkQuickFormat.SetCheck(1);
...
void CBurnerDialog::OnCbnSelchangeComboMode()
{
	// DAO
	int nSelection = m_comboWriteMethod.GetCurSel();
	if (0 == nSelection)
	{
		m_chkCloseDisc.SetCheck(1);
		m_chkCloseDisc.EnableWindow(FALSE);
	}
	// Incremental OR DVD-R DL Layer Jump
	else if (1 == nSelection || 2 == nSelection)
	{
		if (!m_chkCloseDisc.IsWindowEnabled())
			m_chkCloseDisc.SetCheck(1);
...
void CBurnerDialog::OnButtonBrowseClicked() 
{
	...
	try
	{
		...
		ImageTypeFlags::Enum imageType = (ImageTypeFlags::Enum)m_comboImageType.GetItemData(nSel);

		m_ddwRequiredSpace = m_Burner.CalculateImageSize((LPCTSTR)m_strRootDir, imageType);
...
void CBurnerDialog::OnButtonEraseClicked()
{
	Erase();
	UpdateDeviceInformation();
}
...
void CBurnerDialog::OnCheckDvdVideoClicked()
{
	// See also CBurnerDialog::SetImageLayoutFromFolder for other DVD Video related code
	BOOL bChecked = (1 == m_chkVideoDVD.GetCheck());
	if (bChecked)
	{
		// UDF & ISO9660;
		m_comboImageType.SetCurSel(3); 
...
void CBurnerDialog::CreateImage(CString strImageFile)
{
	CreateProgressWindow();
		...
		CreateImageSettings settings;
		{
			...
			int nSel = m_comboImageType.GetCurSel();
			settings.ImageType = (ImageTypeFlags::Enum)m_comboImageType.GetItemData(nSel);

			settings.VolumeLabel = (LPCTSTR)m_strVolume;
			settings.VideoDVD = (BST_CHECKED == m_chkVideoDVD.GetCheck());
		}
	...
	try
	{
		...
		CreateProgressWindow();
			...
			BurnImageSettings settings;
			{
				...
				nSel = m_comboWriteMethod.GetCurSel();
				settings.WriteMethod = (WriteMethod::Enum)m_comboWriteMethod.GetItemData(nSel);
...
void CBurnerDialog::Format()
{
	...
	try
	{
		...
		m_ProgressWindow.SetStatus("Formatting. Please wait...");

			FormatSettings settings;
			{
				settings.Quick = (BST_CHECKED == m_chkQuickFormat.GetCheck());
				settings.Force = true;
			}
...
void CBurnerDialog::Erase()
{
	...
	try
	{
		...
		CreateProgressWindow();
			...
			DWORD dwThreadID = 0;
			m_hCommandThread = CreateThread(NULL, NULL, EraseThread, &threadParam, NULL, &dwThreadID);
			WaitForSingleObject(m_hCommandThreadStartedEvent, INFINITE);
...
DWORD CBurnerDialog::EraseThread(LPVOID pVoid) 
{
...
void CBurnerDialog::UpdateDeviceInformation()
{
	...
	try
	{
		...
		{
			CString sSpeed;
			sSpeed.Format(_TEXT("%1.0fx"), (double)nCurSpeedKB / Speed1xKB::DVD);
		...
		// burn button
		m_btnCreate.EnableWindow(m_ddwCapacity > 0 && m_ddwCapacity >= m_ddwRequiredSpace);
